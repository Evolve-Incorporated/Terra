...
public class DNA  {
    ...
    {
        { "maxEnergy", new float[] {0f, 1f} }, // max energia jaką bedzie miał stworek, na początku jak się go stworzy trzeba bedzie przypisać jego energii tą wartość
        { "speed", new float[] {0f, 1f} }, 
    ...
    [SerializeField]
    public static float mutationRange = 0.5f; // to tez moze byc genem
    ...
    public float GetMoveCost() {
        return 0.1f * (getGene("maxEnergy") + getGene("speed") + getGene("range")) / getGene("reproductionCost"); // tu trzeba uwzglednic jescze jakos maxEnergy bo jak to nie przyczynia sie do zwiekszenia kosztu to kazdy stworek bedzie dazyl do maksymalnej wartosci
    }
    ...
    public void Mutate() {
        ...
        foreach (string key in new List<string>(genes.Keys)) {
            ...
            float mutation = genes[key] * UnityEngine.Random.Range(-mutationRange, mutationRange);
    ...
    private void SetGenes(){
        foreach (KeyValuePair<string, float[]> entry in DNA.DNA_RANGES){
            float value = UnityEngine.Random.Range(entry.Value[0], entry.Value[1]);
